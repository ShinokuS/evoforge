...
final class RampNavigationScenarioTest {
    ...
    private static void assertRamp(
            SimulationRuntime runtime, int x, int y, int z,
            int downDx, int downDy, int downDz,
            int upDx, int upDy, int upDz) {
        int transitions = runtime.view().navigation().transitions(x, y, z);
        String label = "ramp (" + x + "," + y + "," + z + ") mask=" + Integer.toBinaryString(transitions);
        assertTrue(TransitionMask.contains(transitions, downDx, downDy, downDz), label + " missing down transition");
        assertTrue(TransitionMask.contains(transitions, upDx, upDy, upDz), label + " missing up transition");
    }
}
