...
final class MoveToInteractiveController implements ScenarioController {
    ...
    private static final int MAX_VISIBLE_DROP = MoveToScenarioCourse.MAX_STANDING_Z - MoveToScenarioCourse.MIN_STANDING_Z;
    ...
    public ScenarioDiagnostics diagnostics() {
        return MoveToScenarioDiagnostics.snapshot(runtime.view(), mover, plan, state);
    }
    ...
    public boolean secondaryCellAction(int x, int y, int selectedZ) {
        ...
        StandingTarget target = resolveVisibleStandingTarget(x, y, selectedZ);
        if (target == null) {
        ...
        MoveToScenarioPlan candidate = MoveToScenarioRoutes.plan(
                runtime.view(), mover, target.x(), target.y(), target.z());
        var result = MoveToScenarioCommands.start(
                runtime, mover, target.x(), target.y(), target.z());
        if (result.accepted()) plan = candidate;
        state = result.code().value();

        if (result.accepted() && !runtime.view().moveTo().isActive(mover)) {
            MoveToCompletion completion = runtime.view().moveTo().lastCompletion(mover);
            if (completion != null && completion.actionId().equals(result.actionId())) {
    ...
    private StandingTarget resolveVisibleStandingTarget(int x, int y, int selectedZ) {
        LandscapeSliceResolver.Cell slice = slices.resolve(x, y, selectedZ, MAX_VISIBLE_DROP);
        return switch (slice.kind()) {
            case CURRENT_SURFACE -> new StandingTarget(x, y, selectedZ);
            case LOWER_SURFACE -> new StandingTarget(x, y, slice.terrainZ() + 1);
            case SOLID_BODY, EMPTY -> null;
        };
    }

    private record StandingTarget(int x, int y, int z) { }
}
