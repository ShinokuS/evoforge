...
public final class TimedMovementScenario implements VisualizerScenario {
    ...
    public ScenarioSession create() {
        ...
        LandscapeDefinitionId ground = assembly.landscapeDefinition(
                "scenario:movement_ground");
        ObjectDefinitionId slowDefinition = assembly.objectDefinition(
                "scenario:slow_walker");
        ObjectDefinitionId fastDefinition = assembly.objectDefinition(
                "scenario:fast_walker");
        ...
        requireStarted(
                runtime.submit(new MoveStepCommand(slow, -2, -1, 1)),
                "slow walker");
        requireStarted(
                runtime.submit(new MoveStepCommand(fast, -2, 1, 1)),
                "fast walker");

        return new ScenarioSession(
                runtime,
                new ScenarioView(1, -1f, 0f, 0.8f));
    }
