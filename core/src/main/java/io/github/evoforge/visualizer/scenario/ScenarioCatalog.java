...
public final class ScenarioCatalog {
    ...
    /**
     * Compatibility constructor for small tests/custom catalogs that do not
     * need an explicit browser grouping.
     */
    public ScenarioCatalog(List<VisualizerScenario> scenarios) {
        this(List.of(new ScenarioGroup(
                "scenarios",
                "Scenarios",
                requireScenarioList(scenarios))),
                true);
    }

    private ScenarioCatalog(
            List<ScenarioGroup> groups,
            boolean grouped) {

        if (groups == null || groups.isEmpty()) {
            throw new IllegalArgumentException(
                    "scenario groups must not be empty");
        }
        ...
        for (ScenarioGroup group : groupCopy) {
            if (group == null) {
                throw new IllegalArgumentException(
                        "scenario group must not be null");
            }
            if (!groupIds.add(group.id())) {
                throw new IllegalArgumentException(
                        "duplicate scenario group id: " + group.id());
            }
    ...
    public static ScenarioCatalog ofGroups(
            ScenarioGroup... groups) {
        if (groups == null) {
            throw new IllegalArgumentException(
    ...
    public List<ScenarioGroup> groups() {
        return groups;
    }

    private static List<VisualizerScenario> requireScenarioList(
