...
public final class CowForagingScenario implements VisualizerScenario {
    ...
    public String description() {
        return "A hungry autonomous cow perceives Grass and Hay, scores both, and chooses the farther but more valuable Hay without a cow-specific behavior script.";
    }
    ...
    public ScenarioSession create() {
        ...
        assembly.agent(cow, 8, GRAZE);
        assembly.need(cow, HUNGER, 100, 60);
        assembly.satisfiesNeed(grass, HUNGER, 15, GRAZE);
        ...
        assembly.placeObject(grassId, 2, 1, 0);
        assembly.placeObject(hayId, 6, -1, 0);

        SimulationRuntime runtime = assembly.start();
        CowForagingController controller = new CowForagingController(
                runtime, cowId, grassId, hayId, HUNGER);
        return new ScenarioSession(
                runtime,
                new ScenarioView(0, 3f, 0f, 0.85f),
                controller);
    }
}
