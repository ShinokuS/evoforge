 ...
 * fragmentation. The external world frame contributes ordinary ocean control anchors to the same
 * distance field, so boundary water is generated by geometry rather than repainted afterwards.
 * Smooth domain warp and secondary coast detail fade to zero at that frame. A bounded signed-field
 * relaxation then rounds only raster-scale coastal angularity before the final land/water threshold.
 * </p>
 */
final class PlateLandmassSilhouetteAlgorithm implements LandmassSilhouetteAlgorithm {
    ...
    /**
     * Samples the rectangular finite-world frame with ocean sites dense enough that every boundary
     * raster cell is closer to external ocean than to any interior land site. These are ordinary
     * competitors in the plate distance field, not a post-generation mask.
     */
    private static BoundaryOceanAnchors createBoundaryOceanAnchors(
            int width,
            int height,
            int plateSpacing) {
        int targetSpacing = Math.max(1, plateSpacing / 2);
        ...
        if (index != count) {
            throw new IllegalStateException("boundary ocean anchor count mismatch");
        }
            ...
            for (int localX = 0; localX < width; localX++) {
                ...
                if (edgeDistance < guaranteedMargin && coastScore >= 0d) {
                    throw new IllegalStateException(
                            "plate scaffold breached guaranteed external-ocean boundary");
                }
