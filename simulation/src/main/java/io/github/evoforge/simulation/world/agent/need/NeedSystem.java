...
public final class NeedSystem implements NeedLookup {
    ...
    public long level(ObjectId objectId, NeedId needId) {
        ...
        if (index < 0) {
            throw new IllegalArgumentException("need not present on object " + objectId + ": " + needId);
        }
    ...
    private NeedState requireState(ObjectId objectId) {
        NeedState state = states.get(objectId);
        if (state == null) {
            throw new IllegalArgumentException("need state not found: " + objectId);
        }
        return state;
    }
