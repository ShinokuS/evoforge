...
/** Compiles the `soil` definition aspect: pore capacity + permeability. */
public final class SoilPropertiesDefinitionCompiler
        implements DefinitionAspectCompiler<LandscapeDefinitionId> {

    private final SoilPropertiesDefinitions definitions;

    public SoilPropertiesDefinitionCompiler(SoilPropertiesDefinitions definitions) {
        if (definitions == null) {
    ...
    private static int integer(JsonObject data, String field) {
        ...
        if (!primitive.isNumber()) {
            throw new IllegalArgumentException(qualified + " must be an integer");
        }
        try {
