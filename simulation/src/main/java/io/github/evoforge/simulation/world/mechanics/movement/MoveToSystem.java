...
public final class MoveToSystem
    ...
    private void planAndStart(
        ...
        PathSearch search = pathfinder.begin(
                PathQuery.between(
                                ...
                                active.goalY,
                                active.goalZ)
                        .withConstraint(effectivePathConstraint(active)));
        if (search == null) {
            throw new IllegalStateException(
    ...
    private PathTransitionConstraint effectivePathConstraint(
            ActiveMoveTo active) {

        if (traversalConstraint == MoverTraversalConstraint.ALLOW_ALL) {
