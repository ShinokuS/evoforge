...
public final class V15ContinuumLakeBasePageSource implements ContinuumScalarPageSource {
    ...
    private final ContinuumScalarPageSource unrelaxed;
    ...
    public V15ContinuumLakeBasePageSource(
            ...
            ContinuumScalarPageSource unrelaxed,
            V15ContinuumLakeDomainPlan lakes) {
        if (domain == null || continental == null || unrelaxed == null || lakes == null
                || !domain.equals(continental.domain())
                || !domain.equals(unrelaxed.domain())
    ...
    public ContinuumScalarPage materialize(ContinuumSampleWindow window) {
        ...
        ContinuumScalarPage membershipElevation = unrelaxed.materialize(window);
        ...
        for (int sampleY = 0; sampleY < window.height(); sampleY++) {
            ...
            for (int sampleX = 0; sampleX < window.width(); sampleX++, cursor++) {
                ...
                long unrelaxedValue = Math.round(membershipElevation.sample(sampleX, sampleY));
                output[cursor] = value > 0d && lakes.isLake(x, y, unrelaxedValue) ? 0d : value;
            }
        }
