...
public final class AgentSystem implements AgentDecisionLookup {
    ...
    public AgentSystem(ObjectLookup objects, TransformLookup transforms, AgentDefinitions definitions,
            List<AgentOpportunityProvider> providers, MoveToSystem moveTo, MoveToLookup moveToLookup,
            PerceptionLookup perception, SimulationTime time) {
        if (objects == null || transforms == null || definitions == null || providers == null
                || moveTo == null || moveToLookup == null || perception == null || time == null) {
            throw new IllegalArgumentException("agent system dependencies must not be null");
        }
        this.objects = objects; this.transforms = transforms; this.definitions = definitions;
        this.providers = validateProviders(providers); this.moveTo = moveTo; this.moveToLookup = moveToLookup;
        this.perception = perception; this.time = time;
    }
    ...
    public void activate(ObjectId objectId) {
        ...
        if (!definitions.has(object.definitionId())) throw new IllegalArgumentException("object definition is not autonomous: " + object.definitionId());
        if (!transforms.has(objectId)) throw new IllegalStateException("autonomous object must have a transform before activation: " + objectId);
        ...
        byProcessId.put(active.processId, active); byObjectId.put(objectId, active);
    ...
    public void resume(long processId) {
        ...
        if (!objects.isAlive(active.objectId) || !transforms.has(active.objectId)) { deactivate(active); return; }
        if (active.intent != null) continueIntent(active); else decide(active);
    }
    @Override public AgentDecisionTrace lastDecision(ObjectId agentId) { return agentId == null ? null : lastDecisionByObject.get(agentId); }
    @Override public ObjectId currentTarget(ObjectId agentId) {
    ...
    private void decide(ActiveAgent active) {
        if (objects.get(active.objectId) == null) { deactivate(active); return; }
        ...
        lastDecisionByObject.put(active.objectId, new AgentDecisionTrace(time.tick(), active.objectId,
                traces, selected == null ? null : selected.trace));
        if (selected == null) { scheduler.scheduleAfter(IDLE_RECHECK_TICKS, active.processId); return; }
        MoveToStartAttempt attempt = moveTo.start(active.objectId, selected.trace.x(), selected.trace.y(), selected.trace.z());
        if (!attempt.accepted()) { scheduler.scheduleAfter(ACTIVE_POLL_TICKS, active.processId); return; }
    ...
    private List<Candidate> perceiveCandidates(ObjectId agentId) {
        ...
        for (PerceivedObject perceived : snapshot.objects()) {
            for (int providerIndex = 0; providerIndex < providers.size(); providerIndex++) {
                ...
                result.add(new Candidate(providerIndex, new AgentCandidateTrace(provider.id(), perceived.objectId(),
                        perceived.x(), perceived.y(), perceived.z(), perceived.distance(), evaluation.expectedBenefit(),
                        evaluation.score(), evaluation.motivation())));
            }
        }
        return result;
    }
    ...
    private void continueIntent(ActiveAgent active) {
        if (moveToLookup.isActive(active.objectId)) { scheduler.scheduleAfter(ACTIVE_POLL_TICKS, active.processId); return; }
        ActiveIntent intent = active.intent;
        MoveToCompletion completion = moveToLookup.lastCompletion(active.objectId);
        active.intent = null;
        if (completion == null || !intent.actionId.equals(completion.actionId())) throw new IllegalStateException("autonomous MoveTo completion was lost: " + active.objectId);
    ...
    private void deactivate(ActiveAgent active) {
        byProcessId.remove(active.processId, active); byObjectId.remove(active.objectId, active); lastDecisionByObject.remove(active.objectId);
    }
    private void requireScheduler() { if (scheduler == null) throw new IllegalStateException("agent scheduler is not bound"); }
    private static List<AgentOpportunityProvider> validateProviders(List<AgentOpportunityProvider> providers) {
        ...
        for (AgentOpportunityProvider provider : copy) {
            ...
            if (provider.id() == null || provider.id().isBlank()) throw new IllegalArgumentException("opportunity provider id must not be blank");
            if (!ids.add(provider.id())) throw new IllegalArgumentException("duplicate opportunity provider id: " + provider.id());
        }
        return copy;
    }
    private record Candidate(int providerIndex, AgentCandidateTrace trace) { }
    private static final class ActiveAgent {
        private final long processId; private final ObjectId objectId; private ActiveIntent intent;
        private ActiveAgent(long processId, ObjectId objectId) { this.processId = processId; this.objectId = objectId; }
    }
    private static final class ActiveIntent {
        private final int providerIndex; private final ObjectId sourceId; private final MoveToActionId actionId;
        private ActiveIntent(int providerIndex, ObjectId sourceId, MoveToActionId actionId) {
            this.providerIndex = providerIndex; this.sourceId = sourceId; this.actionId = actionId;
        }
    }
}
