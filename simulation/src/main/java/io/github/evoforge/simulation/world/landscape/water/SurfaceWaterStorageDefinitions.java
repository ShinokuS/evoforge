...
/** Material surface-storage capacities declared by landscape definitions. */
public final class SurfaceWaterStorageDefinitions {
    ...
    public void put(
        ...
        if (frozen) {
            throw new IllegalStateException(
                    "surface water storage definitions are frozen");
        }
        if (id == null) {
        ...
        if (present[index]) {
            throw new IllegalStateException(
                    "surface water storage definition already exists: " + id);
        }
    ...
    private void ensureCapacity(
        ...
        if (requiredCapacity <= capacities.length) {
            return;
        }
        int next = Math.max(
                requiredCapacity,
