...
/** Proves V13 mountain composition against the actual accepted historical dense generator. */
final class V13HistoricalOracleParityTest {
    private static final int MIN_Z_CELLS = -12;
    private static final int MAX_Z_CELLS = 80;

    @Test
    void exactContinuumV13MatchesHistoricalDenseGeneratorCellForCell() {
        for (Fixture fixture : new Fixture[] {
                new Fixture(55, 47, 913L),
                new Fixture(64, 51, -4_759_010_560_822_749_572L)
        }) {
    ...
    private static void assertFixtureParity(Fixture fixture) {
        ...
        ElevationField historical = V13MountainTerrainGenerator.standard().generate(genesis);

        V13ContinuumTerrainPlan continuum = V13ContinuumTerrainPlan.prepare(
                domain,
                fixture.seed(),
        ...
        for (int y = 0; y < fixture.height(); y++) {
            ...
            for (int x = 0; x < fixture.width(); x++) {
                ...
                assertEquals(
                        expected,
                        actual,
                        "V13 parity failed at seed=" + fixture.seed()
                                + " x=" + x
                                + " y=" + y
                                + " legacyX=" + legacyX
                                + " legacyY=" + legacyY);
